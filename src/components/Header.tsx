...
const Header: React.FC<HeaderProps> = ({ theme, themeClasses, toggleTheme, setCurrentPage, currentPage, openSearch, currentUser, isAdmin }) => {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center h-20">
                        ...
                        <nav className="hidden lg:flex lg:space-x-2 items-center h-full">
                            {navItems.map(item => (
                                <NavDropdown key={item.path} item={item} themeClasses={themeClasses} setCurrentPage={setCurrentPage} currentPage={currentPage} openSearch={openSearch} />
                            ))}
                            ...
                            >
                                <a 
                                    ...
                                    className={`flex items-center px-3 py-2 rounded-full text-sm font-medium transition-colors ${themeClasses.text} hover:bg-gray-500/10 ${isAdmin ? 'bg-red-500/10 text-red-500' : ''}`}
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-1">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                                    </svg>
                                    {currentUser ? (isAdmin ? 'Admin' : 'User') : 'Login'}
                                </a>

                                {/* Dropdown */}
                                {currentUser && isUserMenuOpen && (
                                    <div className={`absolute right-0 mt-0 w-40 rounded-lg shadow-xl ${themeClasses.cardBg} border ${themeClasses.border} z-50 overflow-hidden`}>
                                        <div className="py-1">
                                            {isAdmin && (
                                                <a 
                                                    href="#admin"
                                                    onClick={(e) => { e.preventDefault(); setCurrentPage('admin'); }}
                                                    className={`block px-4 py-2 text-sm ${themeClasses.text} hover:bg-gray-500/10`}
                                                >
                                                    Dashboard
                                                </a>
                                            )}
                                            <button 
                                                onClick={logout}
                                                className={`block w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-gray-500/10`}
                                            >
                                                Logout
                                            </button>
                                        </div>
