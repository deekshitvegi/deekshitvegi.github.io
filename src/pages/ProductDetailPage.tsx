...
const HighlightIcon: React.FC<{ iconName: string }> = ({ iconName }) => {
    const iconMap: { [key: string]: React.ReactElement } = {
        ...
        'cpu-chip': <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 3v1.5M4.5 8.25H3m18 0h-1.5M4.5 12H3m18 0h-1.5m-15 3.75H3m18 0h-1.5M8.25 19.5V21M12 3v1.5m0 15V21m3.75-18v1.5M19.5 8.25h-1.5m-15 3.75h1.5m15 0h1.5m-15 3.75h1.5m15 0h1.5m-15 3.75h1.5m15 3.75h1.5m-15 3.75h1.5m15 0h1.5" />,
...
const ProductDescription: React.FC<{ description: string, themeClasses: ThemeClasses }> = ({ description, themeClasses }) => {
    const [isExpanded, setIsExpanded] = useState(false);
    const MAX_LENGTH = 300;
    const shouldTruncate = description.length > MAX_LENGTH;

    const displayedText = isExpanded || !shouldTruncate ? description : `${description.substring(0, MAX_LENGTH)}...`;

    return (
        <div className="mt-4">
            <p className="text-lg opacity-90 leading-relaxed">{displayedText}</p>
            {shouldTruncate && (
                <button 
                    onClick={() => setIsExpanded(!isExpanded)} 
                    className={`mt-2 text-sm font-semibold ${themeClasses.accent} hover:underline focus:outline-none`}
                >
        ...
        <div className="space-y-12">
            ...
            <section className={`p-8 rounded-2xl ${themeClasses.cardBg} ${themeClasses.border} border flex flex-col md:flex-row items-center gap-8`}>
                ...
                <div className="md:w-1/2">
                    ...
                    <ProductDescription description={product.description} themeClasses={themeClasses} />

                    <div className="mt-6 flex flex-wrap gap-4">
                        <button className={`${themeClasses.button} px-6 py-3 rounded-lg font-semibold flex-grow sm:flex-grow-0`}>Get a Quote</button>
                        {product.brochureUrl && (
                            <a 
                                href={product.brochureUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                className={`${themeClasses.cardBg} border ${themeClasses.border} px-6 py-3 rounded-lg font-semibold hover:bg-gray-500/10 flex items-center justify-center flex-grow sm:flex-grow-0 transition-colors`}
                            >
