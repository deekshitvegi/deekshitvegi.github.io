...
const ProductsPage: React.FC<ProductsPageProps> = ({ themeClasses, setCurrentPage, categorySlug, initialTabSlug }) => {
    ...
    useEffect(() => {
        if (mainCategory) {
            const targetSection = initialTabSlug
                ? mainCategory.sections.find(s => slugify(s.title) === initialTabSlug)
                : mainCategory.sections[0];

            setActiveTab(targetSection?.title || mainCategory.sections[0]?.title || '');
        }
