...
const FAQSection: React.FC<{ themeClasses: ThemeClasses }> = ({ themeClasses }) => {
    ...
    return (
        <section className="py-12 mt-12 border-t border-gray-200 dark:border-gray-700">
...
const ProductsPage: React.FC<ProductsPageProps> = ({ themeClasses, setCurrentPage, categorySlug, initialTabSlug }) => {
    ...
    useEffect(() => {
        if (mainCategory) {
            // If initialTabSlug is present (from URL), use it. Otherwise default to first section.
            if (initialTabSlug) {
            ...
            // Default fallback
            if (mainCategory.sections.length > 0) {
                setActiveTab(mainCategory.sections[0].title);
            }
        }
    }, [categorySlug, initialTabSlug, mainCategory]);
    ...
    const tabClass = (tabTitle: string) => 
      `px-5 py-2 text-sm font-semibold rounded-full transition-all duration-200 whitespace-nowrap ${
        tabTitle === activeTab
    ...
    const handleTabClick = (sectionTitle: string) => {
        if (categorySlug) {
            const sectionSlug = slugify(sectionTitle);
            setCurrentPage(`products/${categorySlug}/${sectionSlug}`);
        } else {
    ...
    return (
        <div className={`${themeClasses.text} space-y-12`}>
            ...
            <div className={`sticky top-[79px] z-30 ${themeClasses.mainBg} bg-opacity-80 backdrop-blur-sm py-4 -mx-4 px-4 border-b ${themeClasses.border}`}>
                    ...
                    {currentSection.subsections.map((subsection) => (
                        <section key={subsection.name}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                {subsection.products.map((productName, index) => (
                                    <ProductCard 
                                        key={productName} 
                                        name={productName} 
                                        index={index}
